package Backend_Automation_TestCases;
...
import rsa_ECom_ApiTest.CreateOrderDetailsReq;
import rsa_ECom_ApiTest.CreateOrderResponse;
import rsa_ECom_ApiTest.CreateOrdersReq;
import rsa_ECom_ApiTest.LoginRequest;
import rsa_ECom_ApiTest.LoginResponse;
import rsa_ECom_ApiTest.ViewOrdersMainPage;

public class Rest_Ecom_Api_Test

{
	@Test

	public void ecomerceApiAutomation()

	{
		...
		JsonPath jp = ReuseableMethods.rawToJson(addProductResponse);
		...
		String resCreteOrder = createOrderReq.when().post("/api/ecom/order/create-order").then().log().all().extract()
				.response().asString();
		System.out.println(resCreteOrder);
		JsonPath js1 = ReuseableMethods.rawToJson(resCreteOrder);
		String OrderId = js1.get("orders[0]");
		System.out.println(OrderId);

		CreateOrderResponse resCreteOrder1 = createOrderReq.when().post("/api/ecom/order/create-order").then().log()
				.all().extract().response().as(CreateOrderResponse.class);
		String OrderId1 = resCreteOrder1.getOrders().get(0);
		System.out.println(OrderId1);
		String prodOrderId = resCreteOrder1.getProductOrderId().get(0);
		System.out.println(prodOrderId);
		System.out.println(resCreteOrder1.getMessage());
		...
		RequestSpecification vod = given().log().all().spec(viewOrderDetailsReq).queryParam("id", OrderId1);

		String ordeDetailsPage = vod.when().get("/api/ecom/order/get-orders-details/").then().log().all().extract()
				.response().asString();
		JsonPath jp1 = ReuseableMethods.rawToJson(ordeDetailsPage);
		...
		JsonPath js = ReuseableMethods.rawToJson(deleteProdResponse);
