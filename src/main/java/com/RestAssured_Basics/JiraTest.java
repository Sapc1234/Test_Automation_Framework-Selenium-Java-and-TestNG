...
public class JiraTest {
...

	{
		...
		String creteResponse = given().log().all().header("Content-Type", "application/json").header("Authorization",
				...
				.body(API_Payloads.createJiraBug()).when().post("rest/api/3/issue").then().log().all().statusCode(201)
				.extract().response().asString();
		System.out.println(creteResponse);

		JsonPath js = API_ReuseableMethods.rawToJson(creteResponse);
		...
		String fillAtt = given().log().all().pathParam("key", issueId).header("Authorization",
				...
				.asString();
		JsonPath js1 = API_ReuseableMethods.rawToJson(fillAtt);
		...
		JsonPath jso = API_ReuseableMethods.rawToJson(getIssueRes);
